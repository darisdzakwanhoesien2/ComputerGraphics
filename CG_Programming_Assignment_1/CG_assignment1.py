...
class Win(GlutWindow):
	...
	def calc_model(self):
		...
		# Compute time since program started
		elapsed = time.time() - start_time

		# Convert time to rotation angle (e.g., radians or degrees)
		angle = glm.radians(elapsed * 50)  # 50 degrees per second

		# Create a rotation matrix around X-axis
		rotation_x = glm.rotate(glm.mat4(1.0), angle, glm.vec3(1.0, 0.0, 0.0))

		# Assign it to the model matrix
		self.model_matrix = rotation_x
